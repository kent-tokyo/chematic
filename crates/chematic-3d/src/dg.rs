...
// ---------------------------------------------------------------------------
// Connectivity-ordered ring/chain placement (Phase 1, issues #256/#255)
// ---------------------------------------------------------------------------
//
// See `docs/rfcs/dg_connectivity_ordered_placement_rfc.md`. New, additive,
// crate-private engine -- `generate_coords`/`place_rings`/`dfs_place` above
// are completely untouched; this is a parallel implementation, not an
// in-place rewrite (RFC §5, Phase 1). Not yet wired up as `generate_coords`'s
// own implementation or any live caller -- that routing decision is RFC §5's
...
// `dfs_place`'s own unchanged (no grandparent-referenced staggering, per
// the RFC's recorded recommendation to scope this rewrite strictly to the
// ring-anchor defects).
...
/// today's `place_rings` already distinguishes "shared atom" fusion from
/// "direct bond, new island" anchoring.
///
...
/// Conformer Z offset at ring-position `step`, mirroring `place_rings`' own
/// rules (chair/envelope/crown for isolated, non-aromatic, non-fused rings;
/// flat otherwise). `is_fused` is passed explicitly by the caller rather
/// than inferred from `placed` state's timing (this engine's placement
/// order differs from `place_rings`', so that implicit-timing trick isn't
/// reliable here) -- true for a shared-edge/spiro continuation within an
/// already-anchored ring system, false for a system's own first ring.
...
/// the ring's own XY-parallel plane -- see this section's module doc for why
/// only XY matters here). Ported from
...
/// listed atom order. This is issue #255's actual fix: `place_rings`' old
/// `shared_atoms.len() >= 2` branch used a fixed `+y` extension instead,
/// correct only when +y happened to point away from the already-placed
/// ring. Ported from `chematic-depict/layout.rs::place_ring_anchored`
/// (verified there against naphthalene/phenanthrene/pyrene/anthracene via
/// `mol.depict_data()`, see the RFC §3b), with dg.rs's own chemistry-aware
/// bond length substituted for `BOND_LEN`, and a Z coordinate added (the
/// anchor edge's shared height -- both anchor atoms already sit at the same
/// Z in every case this engine produces, since ring planes are always
/// XY-parallel).
#[allow(clippy::too_many_arguments)]
...
/// sidesteps needing any SSSR-order-dependent visiting order at all (the
/// bug `order_rings_by_fusion_adjacency` above exists to work around for
...
/// first ring centered at `(x_offset + r, 0, 0)` (matching `place_rings`'
/// own convention for a component's first-placed ring), then the rest of
/// the system grown via [`grow_ring_system_3d`].
...
///
/// Identical bond-angle/dihedral geometry to [`dfs_place`] above for a plain
/// chain continuation (deliberately duplicated, not shared -- `dfs_place`
/// itself must stay untouched per the RFC's "parallel implementation, never
/// an in-place rewrite" scope). The one real difference: before extending
/// toward an unplaced neighbor, check whether that neighbor belongs to a
/// not-yet-placed ring system (`atom_to_system`). If so, anchor the WHOLE
/// system there (flattening the chosen bond direction's XY component only --
/// see this section's module doc for why) and push its atoms onto
/// `worklist` for the caller to continue growing from, instead of recursing
/// directly -- a ring atom may have several of its own further branches,
/// each needing its own bond-angle/dihedral context, exactly as
/// `place_component`'s own `for atom in component { dfs_place(...) }` loop
/// already does for every ring atom today.
#[allow(clippy::too_many_arguments)]
...
/// Connectivity-ordered ring/chain placement (RFC's Phase 1). Same contract
/// as [`generate_coords`] (non-degenerate, ideal-bond-length-approximating,
/// not physically minimised) -- a parallel implementation, not a
...
mod tests {
    ...
    // -----------------------------------------------------------------
    // generate_coords_connectivity_ordered (Phase 1, issues #256/#255)
    // -----------------------------------------------------------------
    //
    // Acceptance oracle for every fixture below: the RFC's own measured
    // worst-bonded-pair-distance table (docs/rfcs/
    // dg_connectivity_ordered_placement_rfc.md §2). `generate_coords`
    ...
    fn connectivity_ordered_naphthalene_fusion_seam_sane() {
        ...
        // `generate_coords_naphthalene_fusion_seam_known_broken` above).
        let mol = parse("c1ccc2ccccc2c1").unwrap();
    ...
    fn connectivity_ordered_diphenylmethane_chain_bridge_length_1_sane() {
        // #256, shortest bridge: today's `generate_coords` distorts this to
        // 8.0738 Å (see the `..._known_broken` test above).
        let mol = parse("c1ccccc1Cc1ccccc1").unwrap();
    ...
    fn connectivity_ordered_bibenzyl_chain_bridged_ring_islands_sane() {
        // #256, bridge length 2: today's `generate_coords` distorts this to
        // 8.7358 Å.
        let mol = parse("c1ccccc1CCc1ccccc1").unwrap();
