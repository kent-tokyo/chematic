...
//!
//! Keys without a SMARTS pattern follow RDKit's `GenMACCSKeys`:
//! - Key 1  (index 0):   never set (RDKit leaves ISOTOPE unimplemented, `'?'`)
//! - Key 125 (index 124): more than one ring whose bonds are all aromatic,
//!   over RDKit's ring info (symmetrized SSSR) of the RDKit-parity
//!   aromaticity view
...
static MACCS_SMARTS: &[(&str, usize)] = &[
    ...
    ("[#6]=;@[#6](@*)@*", 0), // key 26 - C=C(Q)Q in ring (RDKit MACCSkeys.py verbatim)
    ("[I]", 0),               // key 27 - I
...
pub fn maccs(mol: &Molecule) -> BitVec2048 {
    ...
    // Key 1 (index 0) stays unset: RDKit's MACCS definition marks ISOTOPE
    // as unimplemented ('?') and never sets it.
...
mod tests {
    ...
    #[test]
    fn maccs_isotope_key_is_never_set_like_rdkit() {
        // RDKit MACCSkeys.py defines key 1 (ISOTOPE) as '?' and never sets it
        // (RDKit 2026.03.6: GenMACCSKeys([13CH4]) has no bit 1).
        let labeled = parse("[13CH4]").unwrap();
        assert!(
            !maccs(&labeled).get(0),
            "key 1 must stay unset, as in RDKit"
        );
    }
    ...
    fn maccs_key26_ring_double_bond() {
        // RDKit key 26 is `[#6]=;@[#6](@*)@*`: a ring C=C whose second carbon
        // has two further ring bonds. RDKit 2026.03.6 oracle: octalin (fused
        // ring C=C) sets it; cyclohexene and ethylene do not.
        let fused = parse("C1CCC2=C(C1)CCCC2").unwrap();
        assert!(maccs(&fused).get(25), "fused ring C=C should set key 26");
        let cyclohexene = parse("C1=CCCCC1").unwrap();
        assert!(
            !maccs(&cyclohexene).get(25),
            "cyclohexene must not set key 26"
        );
        let ethylene = parse("C=C").unwrap();
        assert!(!maccs(&ethylene).get(25), "acyclic C=C must not set key 26");
    }
