...
/// 926 torsion entries (Halgren 1996 Table VI). Sorted by (tors_type, type_i, type_j, type_k, type_l).
/// type_i=0 or type_l=0 → wildcard (matches any atom type).
...
/// Look up torsion parameters by MMFF94 numeric atom types.
///
/// Implements MMFF94's fallback hierarchy:
/// 1. Exact match
/// 2. Reversed (tl,tk,tj,ti)
/// 3. Wildcard first atom (0,tj,tk,tl) then wildcard last atom (ti,tj,tk,0)
/// 4. Both wildcards (0,tj,tk,0), then tors_type-generic (0,0,tj,tk,0)
pub fn mmff94_torsion_energy(
    ...
    search(tors_type, ti, tj, tk, tl)
        ...
        // 5: tors_type-generic (type 0)
        .or_else(|| {
            if tors_type != 0 {
                search(0, 0, tj, tk, 0).or_else(|| search(0, 0, rj, rk, 0))
            } else {
                None
            }
        })
