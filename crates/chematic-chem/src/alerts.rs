...
use chematic_core::{Atom, AtomIdx, BondOrder, Element, Molecule, MoleculeBuilder, implicit_hcount};
...
fn add_explicit_hs(mol: &Molecule) -> Molecule {
    ...
    for (_, bond) in mol.bonds() {
        builder.add_bond(bond.atom1, bond.atom2, bond.order).unwrap();
    }
...
fn compiled_patterns() -> &'static [(&'static str, QueryMolecule)] {
    COMPILED.get_or_init(|| {
        PAINS_SMARTS.iter().filter_map(|&(name, smarts)| {
...
mod tests {
    ...
    fn test_pains_caffeine_passes() {
        ...
        assert!(pains_passes(&mol("Cn1cnc2c1c(=O)n(c(=O)n2C)C")),
    ...
    fn test_pains_count_at_most_480() {
        ...
        assert!(n >= 470, "at least 470 of 480 patterns should compile, got {n}");
    }
    ...
    fn test_pains_mannich_base_fails() {
        ...
        assert!(!hits.is_empty(), "Mannich base should trigger PAINS mannich_A");
    }
}
