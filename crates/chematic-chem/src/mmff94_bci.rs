...
fn is_carbonyl_oxygen(mol: &Molecule, idx: AtomIdx) -> bool {
    mol.neighbors(idx).any(|(_, bi)| mol.bond(bi).order == BondOrder::Double)
}
...
pub fn assign_mmff94_type(mol: &Molecule, idx: AtomIdx) -> MmffType {
    ...
    match an {
        6 => {
            if atom.aromatic { return MmffType::Caromatic; }
            ...
            if mol.neighbors(idx).any(|(_, bi)| mol.bond(bi).order == BondOrder::Double) {
        ...
        7 => {
            if atom.aromatic {
                let h = implicit_hcount(mol, idx) as usize
                    + mol.neighbors(idx).filter(|(nb, _)| mol.atom(*nb).element.atomic_number() == 1).count();
                return if h > 0 { MmffType::NarH } else { MmffType::Nar };
            }
            if is_amide_n(mol, idx) { return MmffType::Namide; }
            if mol.neighbors(idx).any(|(_, bi)| mol.bond(bi).order == BondOrder::Double) {
        ...
        8 => {
            if atom.aromatic { return MmffType::Oaromatic; }
            if is_carbonyl_oxygen(mol, idx) { return MmffType::Ocarbonyl; }
            // O-H → hydroxyl
            let h = implicit_hcount(mol, idx) as usize
                + mol.neighbors(idx).filter(|(nb, _)| mol.atom(*nb).element.atomic_number() == 1).count();
            if h > 0 { return MmffType::Ohydroxyl; }
            // Ester: O bonded to a carbonyl C
            let has_carbonyl_nb = mol.neighbors(idx).any(|(nb, _)| {
                mol.atom(nb).element.atomic_number() == 6 && is_carbonyl_c(mol, nb)
            });
            if has_carbonyl_nb { return MmffType::Oester; }
            MmffType::Oether
        }
        16 => {
            if atom.aromatic { return MmffType::Saromatic; }
...
fn bci_typed(a_type: MmffType, b_type: MmffType, order: BondOrder) -> f64 {
    ...
    let (lo, hi, flipped) = {
        let da = a_type as u8;
        let db = b_type as u8;
        if da <= db { (a_type, b_type, false) } else { (b_type, a_type, true) }
    };
    let ctx = BondCtx::from_order(order);
...
mod tests {
    ...
    fn total_q(q: &[f64]) -> f64 { q.iter().sum() }

    #[test]
    fn test_typed_charge_conservation() {
        for smi in &["CCO", "CC(C)=O", "CC(=O)N", "c1ccccc1", "CC(=O)O", "CN",
                     "CS(=O)(=O)N", "CC(=O)[O-]", "[NH4+]"] {
            let mol = parse(smi).unwrap();
            let q = mmff94_charges_typed(&mol);
            let fc: f64 = mol.atoms().map(|(_, a)| a.charge as f64).sum();
            assert!((total_q(&q) - fc).abs() < 1e-9,
                "charge not conserved for {smi}: sum={:.6} fc={fc}", total_q(&q));
        }
    }
    ...
    fn test_typed_atom_type_discrimination() {
        ...
        let types_ac: Vec<MmffType> = (0..acetone.atom_count())
            ...
            .collect();
        assert!(types_ac.iter().any(|&t| t == MmffType::Ccarbonyl), "acetone should have Ccarbonyl");
        assert!(types_ac.iter().any(|&t| t == MmffType::Ocarbonyl), "acetone should have Ocarbonyl");
    }
    ...
    fn test_typed_ester_vs_ether_oxygen() {
        ...
        let ester_types: Vec<_> = (0..ester.atom_count())
            ...
            .collect();
        assert!(ester_types.iter().any(|&t| t == MmffType::Oester),
    ...
    fn test_typed_aromatic_nitrogen_types() {
        ...
        assert!(pyr_types.iter().any(|&t| t == MmffType::Nar),
            "pyridine N should be Nar");
        assert!(rol_types.iter().any(|&t| t == MmffType::NarH),
        ...
        let n_q_pyr = pyridine.atoms()
            .filter(|(_, a)| a.element.atomic_number() == 7)
            .map(|(i, _)| q_pyr[i.0 as usize]).next().unwrap();
        let n_q_rol = pyrrole.atoms()
            .filter(|(_, a)| a.element.atomic_number() == 7)
            .map(|(i, _)| q_rol[i.0 as usize]).next().unwrap();
        // Both should be in reasonable range
        assert!(n_q_pyr < 0.0, "pyridine N should be negative ({n_q_pyr:.3})");
    ...
    fn test_typed_ester_o_charge_differs_from_ether_o() {
        ...
        if let (Some(ei), Some(oi)) = (ester_o_idx, ether_o_idx) {
            assert!((q_ester[ei] - q_ether[oi]).abs() > 0.001,
                "ester O ({:.4}) and ether O ({:.4}) should differ", q_ester[ei], q_ether[oi]);
        }
    ...
    fn test_typed_vs_element_bci_carbonyl() {
        ...
        let o_idx = acetone.atoms()
            .find(|(_, a)| a.element.atomic_number() == 8)
            .map(|(i, _)| i.0 as usize).unwrap();
        assert!(q_t[o_idx] < -0.3, "typed acetone O should be < -0.3 (got {:.3})", q_t[o_idx]);
        assert!(q_b[o_idx] < -0.3, "bci   acetone O should be < -0.3 (got {:.3})", q_b[o_idx]);
    }
}
