//! Emit RDKit-compatible ECFP4 sparse raw-identifier counts as JSONL.

use serde_json::json;
use std::io::{self, BufRead, Write};

fn sparse_counts(mol: &chematic_core::Molecule) -> Vec<(u32, u32)> {
    let result = chematic_fp::rdkit_morgan_ecfp4_experimental(mol).expect("handled by caller");
    let mut values: Vec<_> = result.sparse_counts.into_iter().collect();
