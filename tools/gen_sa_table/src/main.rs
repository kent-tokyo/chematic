...
const CORPUS: &[&str] = &[
    ...
    "CC(=O)O", "CCC(=O)O", "CCCC(=O)O", "CCCCCCCCCCCCC(=O)O",
    "CC(=O)N", "CC(=O)NC",
    "CC(=O)OC",
    "OC(=O)CO",
    "OC(=O)CC(=O)O",
...
fn main() {
    ...
    println!("// Corpus: {mol_count} molecules | {} unique fragment environments", entries.len());
