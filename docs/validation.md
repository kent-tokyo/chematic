...
| Num stereocenters (legacy) | 99.98%† | exact | 4998/4999 vs `CalcNumAtomStereoCenters` |
| Num stereocenters (new CIP) | 98.7% | exact | 4932/4999 vs `FindPotentialStereo` |

19 of 19 tested metrics reach ≥98.7% on the 4,999-molecule ChEMBL corpus.
...
*† Stereocenters (legacy): the 1 discrepancy (chematic=4, legacy=2) is a polyester where chematic and `FindPotentialStereo` both return 4; legacy `CalcNumAtomStereoCenters` misses 2 pseudoasymmetric centres. Stereocenters (new CIP): `FindPotentialStereo` counts cage/bridgehead atoms as potential stereocenters in 67 molecules where both chematic and legacy agree they are not (false positives in the new-CIP oracle for cage compounds).*
