...
| Multi-conformer ensemble generation | **Partial** — sound engine now exists (Rust core only, not yet in Python/WASM) | **Updated 2026-08-23 (A2, PR #371, `cc0c0b1`)**: a new, sound multi-conformer path now exists — `embed_ensemble_v2` (`crates/chematic-3d/src/ensemble_v2.rs`), a genuinely new outer loop calling `embed_pipeline_v2` per deterministically-derived seed (`derive_attempt_seed`, reused from `distance_geometry_v2.rs`), grouping kept conformers by `actual_force_field_used` and ranking by ascending energy *within* each group only (MMFF94/UFF energies are never compared across a fallback boundary), with automorphism-aware duplicate pruning by default. Full per-attempt provenance (`ConformerDisposition::Kept`/`PrunedAsDuplicate{representative_attempt_index, rmsd, symmetric}`) and typed `EnsembleTermination::{Completed, TimedOut}`. **Not yet callable from Python or WASM**, and not yet benchmarked against RDKit's `etkdgv3_best_of_n` arm — Rust-core wiring only this round. The legacy, public `conformer_ensemble()` API (`crates/chematic-py`, wrapping `generate_conformer_ensemble_with_config`) is **completely untouched and still has its live defect**: sits on the older `etkdg.rs` path, no seed control (process-global atomic counter, not caller-reproducible), no energy ranking (RMSD-uniqueness only), silent minimizer failures (no `Result`). Confirmed still live: MMFF94 silently contributes zero energy/gradient for atom-type pairs its tables don't cover — on a halomethane case (`[C@H](F)(Cl)Br`), central-C-to-halogen distances land at 8.9-11.3 Å under `conformer_ensemble(1, 0.0, 'mmff94', 0.0)`, vs. a real ~1.4-1.9 Å (`validation/openeye_advantage_fixtures.jsonl`'s `oe-01` row). Rewiring or deprecating this legacy path in favor of `embed_ensemble_v2` is a separate, not-yet-authorized decision. |
...
| RMSD/TFD/stereo-retention/runtime reported separately | Ahead of chematic's own current public claims | True in `validation/results/mmff94_bci_gap_227_phase2_report.md` (RMSD mean 1.685 Å, TFD mean 0.2228, stereo satisfied/violated counted separately, per-stage runtime) on the project's 265-molecule corpus (`227` in that report's name is issue #227, not a molecule count — a naming misreading this RFC's own A1 follow-up ledger briefly introduced and then corrected). The doc gap this row originally flagged (`docs/benchmark.md`/`docs/rdkit-migration.md`/the v0.19.0 CHANGELOG saying "no RMSD/TFD comparison exists yet") was already fixed in this PR's own second commit (`9a4ce51`) — no longer an open gap. A1 (2026-08-23) independently reconfirmed both figures fresh on the same corpus/arm (RMSD mean 1.678 Å, TFD mean 0.223) and additionally measured TFD via `scripts/pipeline_v2_vs_rdkit_tfd_227.py` (RDKit's own `GetTFDBetweenMolecules`, not hand-rolled) for the first time as part of *this* benchmark's own reporting, not just the separate Phase 2 report. |
| Best-of-N conformer selection | **Missing** | The project's own benchmark script gives RDKit a `BEST_OF_N=10` UFF-optimized-then-lowest-energy arm; chematic has zero equivalent arm in that benchmark — every chematic measurement there is single-embed. A2's `embed_ensemble_v2` (2026-08-23) makes N-attempt, energy-ranked selection available in the Rust core, but no ensemble-mode arm has been added to this benchmark to compare against RDKit's arm on equal footing yet. |
...
| Energy-ranked conformer selection | **Partial** — implemented in Rust core, not yet exposed to Python/WASM | `embed_ensemble_v2` (2026-08-23, A2) ranks kept conformers by ascending energy within each `actual_force_field_used` group. Not usable outside Rust yet — see the Omega-equivalent ensemble row above. |
...
| Omega | Partial (sound single-conformer AND sound Rust-core ensemble; legacy Python-exposed ensemble path still broken/unsound) | **A1 done, A2 done (Rust core) — Python/WASM bindings next** |
