...
|---|---|---|---|
| MOL/SDF | `SdfFileReader<R: BufRead>` — true streaming `Iterator` | `iter_sdf` and `iter_sdf_batched` are file-backed streaming iterators; batches are lazy, cancellable, and expose a progress manifest with seen/emitted/rejected counts. WASM additionally exposes bounded resumable `sdf_records_batch_json`, `xyz_frames_batch_json`, and `extxyz_frames_batch_json` over in-memory strings; stopping before the next offset is the cancellation boundary, not file-backed streaming. The WASM XYZ convenience path can retain a malformed frame and continue only when a later count-line boundary is unambiguous; core file-backed readers remain fail-stop. | materializes |
