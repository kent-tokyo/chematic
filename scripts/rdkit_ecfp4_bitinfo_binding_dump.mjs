...
for (const [index, raw] of input.split(/\r?\n/).entries()) {
  ...
  try {
    ...
    const folded_bit_info = Object.entries(detail.foldedBitInfo)
      .map(([bit, entries]) => [Number(bit), entries.map(([atom, radius]) => [atom, radius]).sort((a, b) => a[0] - b[0] || a[1] - b[1])])
      .sort((a, b) => a[0] - b[0]);
    console.log(JSON.stringify({ index, smiles, status: "ok", folded_bit_info }));
    mol.free();
  } catch (error) {
