#!/usr/bin/env python3
"""Measure RDKit-compatible ECFP4 raw bitInfo parity across bindings."""
...
REPORT = ROOT / "validation" / "results" / "rdkit-ecfp4-raw-bitinfo-cross-binding-parity-5000-v1.0.13.json"
...
def python_records(corpus: str) -> list[dict]:
    ...
    for index, raw in enumerate(corpus.splitlines()):
        ...
        try:
            ...
            _, _, raw_info, _ = mol.rdkit_ecfp4_detail()
            pairs = [[int(identifier), [[int(atom), int(radius)] for atom, radius in sorted(entries)]] for identifier, entries in sorted(raw_info.items())]
            rows.append({"index": index, "smiles": smiles, "status": "ok", "raw_bit_info": pairs})
...
def compare(left: list[dict], right: list[dict]) -> dict:
    ...
    for index, (a, b) in enumerate(zip(left, right, strict=True)):
        ...
        if same and a["status"] == "ok": same = a["raw_bit_info"] == b["raw_bit_info"]
        if same: matches += 1
...
def main() -> int:
    ...
    report = {
        ...
        "contract": "same chematic source SMILES, RDKit-compatible ECFP4 raw identifier -> atom/radius provenance",
        ...
        "boundary": ["This checks raw bitInfo binding parity for chematic's RDKit-compatible implementation, not an independent RDKit oracle run."],
    }
