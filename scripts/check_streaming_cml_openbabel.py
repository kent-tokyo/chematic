#!/usr/bin/env python3
"""Check CML record/failure accounting against Open Babel's CLI boundary."""
...
def main() -> int:
    ...
    path = args.cml if args.cml.is_absolute() else ROOT / args.cml
    ...
    for _ in range(args.repeats):
        result = started([args.openbabel, "-icml", str(path), "-osmi", "-O", "/dev/null"], check=False, text=True, capture_output=True)
    ...
    report = {"schema_version": 1, "target_version": "1.0.9", "format": "cml", "fixture": {"path": str(path.relative_to(ROOT)), "bytes": len(payload), "sha256": hashlib.sha256(payload).hexdigest()}, "repeats": args.repeats, "expected_records": expected, "rows": {"chematic": chematic, "openbabel": {"records": converted, "failures": failures, "input_bytes": len(payload) * args.repeats, "comparison_boundary": "Open Babel CLI process per repetition; startup and conversion included"}}, "comparison_boundary": {"chematic": "Rust CML materialized one-shot parser", "openbabel": "Open Babel CML CLI conversion per repetition"}}
    if errors:
