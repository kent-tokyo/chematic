...
def main() -> None:
    ...
    rdkit_ver = data.get("rdkit_version", "?")
    corpus = data["corpus"]
    m = data["metrics"]
    n = corpus["total"]
    ...
    # These come from the oracle-vs-oracle disagreement counters (legacy vs new-CIP RDKit
    # itself), not the chematic-vs-oracle delta fields (nsc_leg/nsc_new "over"/"under") --
    # those are a different comparison and can diverge from these, as a prior version of
    # this script incorrectly assumed.
    oracle_leg_true_under = stereo_info.get("oracle_disagree_legacy_under", "?")
    oracle_new_true_over = stereo_info.get("oracle_disagree_new_cip_over", "?")

    nh = m["nh_smarts"]
    nh_agree = nh["agreement_pct"]
    nh_match = round(nh_agree * n / 100)

    # Real minimum across every row actually shown in the table below (not a
    # hardcoded floor) -- used for the "N of N metrics reach >=X%" summary line.
    table_keys = ["mw", "heavy_atom_count", "hbd", "hba", "tpsa", "mr", "fsp3", "arc",
                  "num_aliphatic_rings", "num_saturated_rings", "rotatable_bonds",
                  "num_heteroatoms", "num_spiro_atoms", "num_bridgehead_atoms",
                  "num_amide_bonds", "num_aromatic_heterocycles"]
    table_pcts = [m[k]["agreement_pct"] for k in table_keys]
    table_pcts += [m["logp"]["agreement_pct"], nh_agree,
                   nsc_leg["agreement_pct"], nsc_new["agreement_pct"]]
    metrics_min_pct = min(table_pcts)
    metrics_count = len(table_pcts)

    descriptor_rows = "\n".join([
        row("Molecular weight",      "mw",                 "±0.01 Da", "vs `Descriptors.MolWt`"),
...
**Environment:** Python 3.13.6, Apple M4, chematic v{ver} (commit `{{commit}}`), RDKit {rdkit_ver}, measured {ts}
...
{metrics_count} of {metrics_count} tested metrics reach ≥{metrics_min_pct:.1f}% on the {n:,}-molecule ChEMBL corpus.
...
*Validation corpus: ChEMBL-derived {n:,}-molecule SMILES set (`scripts/chembl_accuracy_corpus_4999.smi`, 5,000 raw lines; `csv.DictReader` treats the first line as a header, so {n:,} molecules are actually evaluated -- see the Reproduce section). Details: [`benchmark.md`](benchmark.md) · [`rdkit-comparison.md`](rdkit-comparison.md)*
"""
