#!/usr/bin/env python3
"""Measure RDKit-compatible torsion byte parity across Rust, Python, WASM."""
...
REPORT = ROOT / "validation" / "results" / "rdkit-torsion-cross-binding-parity-5000-v1.0.9.json"
...
def python_records(corpus: str) -> list[dict]:
    ...
    for index, raw in enumerate(corpus.splitlines()):
        ...
        try:
            ...
            rows.append({"index": index, "smiles": smiles, "status": "ok", "torsion_hex": mol.rdkit_torsion_fp().hex()})
...
def compare(left: list[dict], right: list[dict]) -> dict:
    ...
    for index, (a, b) in enumerate(zip(left, right, strict=True)):
        ...
        if same and a["status"] == "ok": same = a["torsion_hex"] == b["torsion_hex"]
        if same: matches += 1
...
def main() -> int:
    ...
    bindings = {
        "rust": run(["cargo", "run", "-p", "chematic-chem", "--release", "--offline", "--example", "rdkit_torsion_binding_dump"], corpus),
        "python": python_records(corpus),
        "node_wasm": run(["node", "scripts/rdkit_torsion_binding_dump.mjs"], corpus),
    }
    ...
    report = {
        ...
        "contract": "same chematic source SMILES, RDKit-compatible hashed torsion 2048-bit LSB-first bytes",
        ...
        "boundary": ["This is cross-binding parity for chematic's RDKit-compatible implementation, not an independent RDKit oracle run."],
    }
