#!/usr/bin/env python3
"""Evaluate a frozen TPSA candidate against one provenance-bound cohort split."""
...
def main() -> int:
    ...
    parser.add_argument("--strict-tolerance", type=float, default=1e-6)
    ...
    with raw_path.open("w", encoding="utf-8") as raw:
        for index, (source, record) in enumerate(zip(smiles, records, strict=True)):
            ...
            if rd_mol is None or record.get("status") != "ok":
                parse_failures += 1
                raw.write(json.dumps({"index": index, "status": "parse_failure"}) + "\n")
                continue
            expected = float(rdMolDescriptors.CalcTPSA(rd_mol, includeSandP=True))
            actual = float(record["descriptors"]["tpsa"])
            delta = abs(actual - expected)
            max_abs_error = max(max_abs_error, delta)
            matched = delta <= args.strict_tolerance
            strict_matches += int(matched)
    ...
    passed = parse_failures == 0 and mismatches == 0 and strict_matches == len(smiles)
    attestation = manifest.get("unused_data_attestation") or {}
    summary = {
        "schema_version": 1,
        "profile": "tpsa_frozen_candidate_evaluation_v1",
        "status": (
            "passed" if passed else "rejected"
        ) if args.split == "sealed_holdout" else ("development_passed" if passed else "development_failed"),
        "scope": (
            "One-time sealed TPSA evaluation; raw input and per-row output remain local-only and exposed."
        ...
        "accounting": {
            "parsed": len(smiles) - parse_failures,
    ...
    print(json.dumps({"status": summary["status"], **summary["tpsa"]}, sort_keys=True))
