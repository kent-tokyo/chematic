#!/usr/bin/env python3
"""Measure RDKit-compatible path byte parity across Rust, Python, and WASM."""
...
REPORT = ROOT / "validation" / "results" / "rdkit-path-cross-binding-parity-5000-v1.0.9.json"


def jsonl(value: str) -> list[dict]: return [json.loads(line) for line in value.splitlines() if line.strip()]
...
def python_records(corpus: str) -> list[dict]:
    ...
    for index, raw in enumerate(corpus.splitlines()):
        ...
        try:
            ...
            rows.append({"index": index, "smiles": smiles, "status": "ok", "path_hex": mol.path_fp().hex()})
...
def compare(left: list[dict], right: list[dict]) -> dict:
    ...
    for index, (a, b) in enumerate(zip(left, right, strict=True)):
        same = a["status"] == b["status"] and (a["status"] != "ok" or a["path_hex"] == b["path_hex"])
        if same: matches += 1
...
def main() -> int:
    ...
    bindings = {
        "rust": run(["cargo", "run", "-p", "chematic-chem", "--release", "--offline", "--example", "rdkit_path_binding_dump"], corpus),
        "python": python_records(corpus),
        "node_wasm": run(["node", "scripts/rdkit_path_binding_dump.mjs"], corpus),
    }
    ...
    report = {
        ...
        "contract": "same chematic source SMILES, RDKit-compatible path 2048-bit LSB-first bytes",
        ...
        "boundary": ["This is cross-binding parity for chematic's RDKit-compatible path implementation, not an independent RDKit oracle run.", "Native topo_path remains a separate operation."],
    }
