#!/usr/bin/env python3
"""Measure RDKit-compatible ECFP4 folded bitInfo parity across bindings."""
...
REPORT = ROOT / "validation" / "results" / "rdkit-ecfp4-bitinfo-cross-binding-parity-5000-v1.0.13.json"
...
def python_records(corpus: str) -> list[dict]:
    ...
    for index, raw in enumerate(corpus.splitlines()):
        ...
        try:
            ...
            _, _, _, folded = mol.rdkit_ecfp4_detail()
            pairs = [[int(bit), [[int(atom), int(radius)] for atom, radius in sorted(entries)]] for bit, entries in sorted(folded.items())]
            rows.append({"index": index, "smiles": smiles, "status": "ok", "folded_bit_info": pairs})
...
def compare(left: list[dict], right: list[dict]) -> dict:
    ...
    for index, (a, b) in enumerate(zip(left, right, strict=True)):
        ...
        if same and a["status"] == "ok": same = a["folded_bit_info"] == b["folded_bit_info"]
        if same: matches += 1
...
def main() -> int:
    ...
    bindings = {
        "rust": run(["cargo", "run", "-p", "chematic-chem", "--release", "--offline", "--example", "rdkit_ecfp4_bitinfo_binding_dump"], corpus),
        "python": python_records(corpus),
        "node_wasm": run(["node", "scripts/binding_dump.mjs", "rdkit-ecfp4-bitinfo"], corpus),
    }
    ...
    report = {
        ...
        "contract": "same chematic source SMILES, RDKit-compatible ECFP4 folded bit -> atom/radius provenance",
        ...
        "boundary": ["This checks folded bitInfo binding parity for chematic's RDKit-compatible implementation, not an independent RDKit oracle run.", "Raw identifier bitInfo remains a separate provenance gate."],
    }
