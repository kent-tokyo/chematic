#!/usr/bin/env python3
"""Measure native topological-path byte parity across Rust, Python, and WASM."""
...
REPORT = ROOT / "validation" / "results" / "topo-path-cross-binding-parity-5000-v1.0.9.json"
...
def python_records(corpus: str) -> list[dict]:
    ...
    for index, raw in enumerate(corpus.splitlines()):
        ...
        try:
            ...
            rows.append({"index": index, "smiles": smiles, "status": "ok", "topo_path_hex": mol.topo_path_fp().hex()})
...
def main() -> int:
    ...
    bindings = {
        "rust": run(["cargo", "run", "-p", "chematic-chem", "--release", "--offline", "--example", "topo_path_binding_dump"], corpus),
        "python": python_records(corpus),
        "node_wasm": run(["node", "scripts/topo_path_binding_dump.mjs"], corpus),
    }
    ...
    report = {
        ...
        "contract": "same chematic source SMILES, native topo_path 2048-bit LSB-first bytes",
        ...
        "boundary": ["This is native chematic binding parity, not an RDKit topological fingerprint accuracy claim.", "RDKit-compatible path parity is a separate operation."],
    }
