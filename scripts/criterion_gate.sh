...
run_point_estimate() {
  ...
  # Issue #70 hosted calibration only. When explicitly enabled on the
  # throwaway calibration branch, contend with side B on the same CPU. This
  # deliberately biases both the real candidate and the independently-built
  # main-null B arm so the production null-control logic must downgrade a
  # reproduced benchmark regression to environment-inconclusive. Never merge
  # this injection into main.
  local -a runner=()
  local burner_pid=""
