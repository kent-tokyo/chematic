#!/usr/bin/env python3
"""Measure RDKit-compatible ECFP4 parity across Rust, Python, and WASM."""
...
def python_records(corpus: str) -> list[dict]:
    ...
    for index, raw in enumerate(corpus.splitlines()):
        ...
        try:
            ...
            rows.append({"index": index, "smiles": smiles, "status": "ok", "ecfp4_hex": mol.rdkit_ecfp4().hex()})
...
def compare(left: list[dict], right: list[dict]) -> dict:
    ...
    for index, (a, b) in enumerate(zip(left, right, strict=True)):
        ...
        if same and a["status"] == "ok":
            same = a["ecfp4_hex"] == b["ecfp4_hex"]
        if same:
            matches += 1
        elif len(differences) < 10:
            differences.append({"index": index, "left": a, "right": b})
